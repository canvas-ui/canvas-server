...
Optional user-authored free-text `comment` on every document. Implemented across synapsd, embedd, Workspace, and web UI.
...
- **Embedding**: reserved `chunkId: -1` (`embedd/src/constants.js` COMMENT_CHUNK_ID) in the **text** space. Worker (`embedd/src/index.js` `#handle` + `#embedComment`) bundles it with content chunks when the doc routes to text, else does a standalone text upsert; tracks a `written` Set so the seen-`[]` loop never wipes the comment row. `resolveEmbeddingInput` attaches `comment` to every return shape. `getUnembeddedDocIds('text')` unions `feature/has-comment` so commented non-embeddable docs (photos/files/tabs) join the text gap.
...
To re-embed an already-commented photo after deploying the fix: re-save the comment (fires `document.updated` → live enqueue) or run embedd reconcile (photo is in the text gap, not seen-marked since the job threw).
