...
export const HOOK_EVENTS = Object.freeze([
    ...
    { name: 'document.updated', document: true, description: "A document was updated or re-linked. The payload's `reason` says which: 'content' (the document itself changed — carries the document) or 'membership' (only tree placement changed — carries the CHANGED paths in `changed`, no document; DEPRECATED, bind document.linked instead). Rules can match on it with `when.reason`. Batch updates fan out here per document (batch:true set).", payload: '{ id, reason, document?, changed?, context, directory, batch?, batchCount? }' },
    ...
    { name: 'document.linked', document: true, description: 'A document was linked into tree path(s). Carries the full document (unlike the membership-only document.updated a link also emits), so rules can match on content — e.g. "email linked under /projects/x → triage agent". `changed` names the paths it was just filed into, and a `path` condition matches against them. Bulk links fan out here per document (batch:true set).', payload: '{ id, document, changed: { context, directory, features }, batch?, batchCount? }' },
    { name: 'document.unlinked', document: true, description: 'A document was removed from tree path(s) but still exists in the index. Carries the full document; `changed` names the paths it LEFT. Bulk unlinks fan out here per document.', payload: '{ id, document, changed: { context, directory, features }, recursive?, batch?, batchCount? }' },
    { name: 'document.linked.batch', document: false, description: 'A batch of documents was linked in one go (multi-select "Link to", folder import). Fires once per batch — per-document logic fans out via document.linked.', payload: '{ ids, count, changed: { context, directory, features } }' },
    { name: 'document.unlinked.batch', document: false, description: 'A batch of documents was unlinked in one go (once per batch; per-document logic fans out via document.unlinked)', payload: '{ ids, count, changed: { context, directory, features } }' },
    { name: 'document.removed', document: false, description: 'A document was unlinked from paths (id-only compat event; DEPRECATED — bind document.unlinked for automation)', payload: '{ id | ids, changed }' },
...
export const HOOK_CONTEXT_API = Object.freeze([
    ...
    { name: 'classify', signature: 'ctx.classify(target?)', description: 'Classify the event document (default) or another payload/raw doc. Returns predicates like isEmail()/isYoutube()/isPdf()/inPath() plus fields url/host/from/subject/mime/paths; pathMatches(prefix) lists every placement under a prefix with its remainder ({ tree, path, rel }). Never throws.' },
