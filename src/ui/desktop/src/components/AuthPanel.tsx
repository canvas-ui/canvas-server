...
export function AuthPanel({
  ...
  const formData = useMemo<AuthFormData>(
    () => ({ serverUrl: normalizeUrl(serverUrl), email, password, token, mode }),
  ...
  return (
    <form
      ...
      onSubmit={(event) => { event.preventDefault(); if (!busy) onLogin(formData) }}
    >
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <Input
            ...
            onBlur={() => { const n = normalizeUrl(serverUrl); if (n !== serverUrl) setServerUrl(n) }}
            placeholder="canvas.example.com"
          />
          <div className="text-xs text-muted-foreground">https:// is added automatically if no protocol specified</div>
        </div>

        <div className="flex gap-2">
          <Button type="button" variant={mode === 'password' ? 'default' : 'outline'} onClick={() => setMode('password')} className="flex-1">User + Pass</Button>
          <Button type="button" variant={mode === 'token' ? 'default' : 'outline'} onClick={() => setMode('token')} className="flex-1">App Token</Button>
        </div>
